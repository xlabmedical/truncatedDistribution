...
#
  def sample(self,M=()):
    ...
    M_shape=M
    M=tf.reduce_prod(tf.reshape(M,[-1]))
    print(sess.run(M))
  #
    to_sample=tf.concat(([M],tf.shape(self.xaxis)[1:]),axis=0)
  ...
  #
    to_tile_x=tf.concat([[1],[M],tf.ones(tf.size(tf.shape(self.xaxis)[1:]),dtype=tf.int32)],axis=0)
  ...
  #
    ...
    print(sess.run(M_shape))
    return tf.squeeze(tf.reshape(res,tf.concat([M_shape,tf.shape(res)[1:]],axis=0)))
#
  def cdf(self,X):
